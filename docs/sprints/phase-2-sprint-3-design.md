...
- Hover state tracking (`HoveredWord`, `last_hovered_word`) with cached hover state in TerminalPane
...
- Ctrl/Cmd+hover displays URL in terminal footer (non-intrusive overlay)
- Ctrl/Cmd+click opens URLs in default browser
- Works with http://, https://, git://, ssh://, file://, etc.
- Hover state visible via cursor change
- No false positives on terminal text (URL regex only in Sprint 2.3; defer path regexes)
...
- **Missing:** Safe hover state caching for hover effects (avoid entity reads during render)
...
1. Keep `path_hyperlink_regexes` empty in Sprint 2.3 to avoid false positives.
2. Defer path regex configuration to Sprint 2.4+ with an explicit setting toggle.
...
1. **Cache hover state** (add field to `TerminalPane`):

```rust
// In TerminalPane struct:
hovered_url: Option<String>,
```

Update on events (in `handle_navigation_target`):
...
2. **Enhance rendering** (modify `render_terminal_content()`):
...
fn render_terminal_content(&self, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    if let Some(tab) = tabs.get(active_tab_index) {
        ...
        div()
            ...
            .when(self.hovered_url.is_some(), |d| {
                d.child(
                    div()
                        .absolute()
                        .bottom_0()
                        ...
                        .child(format!("🔗 {}", self.hovered_url.clone().unwrap_or_default()))
                )
            })
    } else {
        // ... loading state ...
...
3. **Add cursor change on hover** (modify `render()`):

```rust
fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
    // Check if hovering over a URL (cached state)
...
**Testing:**
- Ctrl/Cmd+hover shows URL at bottom of terminal (from cached state)
...
┌──────────────────────────────────────────────────────────────────┐
│  TerminalPane.render()                                           │
│  - Read cached hover URL from TerminalPane                       │
│  - Set cursor_pointer() when hovering URL                        │
│  - Display URL in bottom footer overlay                          │
└──────────────────────────────────────────────────────────────────┘
...
- [ ] Keep `path_hyperlink_regexes` empty (URL regex only)
...
- [ ] Modify `render_terminal_content()` to show cached hovered URL
...
| False positive URL detection | Low | Low | Use URL regex only in Sprint 2.3; defer path regexes |
...
-- `src/terminal/pane.rs`
  - Lines 75-90: TerminalBuilder configuration (keep path regexes empty)
